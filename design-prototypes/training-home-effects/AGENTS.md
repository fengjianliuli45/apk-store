...
## Stopwatch prototype decisions

- This folder is an approval-only web prototype. Do not modify the production Flutter app from here.
- The selected visual target is the 2026-08-27 training-home concept with a Klarna-like floating bottom dock.
- Preserve the five destinations in this order: 计划、数据、训练、社区、我的.
- Keep the circular voice core with subtitle and the separate low-emphasis 记录饮食 shortcut.
- Background motion should feel like a breathing training pod: slow mist, sparse particles, subtle flow lines and state-responsive ripples; provide reduced-motion and intensity controls.
